...
## Step 0 — Generate synthetic test figures

The test figures in `lineformer_test/` are already committed to the repository. This step is only needed if you want to regenerate them with different parameters.
...
Key parameters in `generate_figures.py`:
...
Runs LineFormer on every image in `lineformer_test/` and draws each detected trace in a distinct colour.
...
> **Note:** This script uses a recursive glob. Keep `lineformer_test/` clean of already-traced images to avoid accidental double-tracing.

---

### 2. Batch inference — black lines + JSON trace data (recommended for analysis)

Runs LineFormer on the three test sub-sets (`bw/`, `bw_solid/`, `color/`) and saves a PNG and a JSON file of raw `[{x, y}, …]` coordinates per figure.
...
Loads the JSON trace files from Step 2 and computes two quality metrics per figure:

| Metric | Description |
|--------|-------------|
| **Kink count** | Number of turning angles > 45° along a trace (subsampled every 10 px). |
| **Curvature Spike Ratio (CSR)** | `max(|θᵢ₊₁ − θᵢ₋₁| / 2) / (median + 1°)`. Smooth curves yield CSR ≈ 1–5; stitched traces yield CSR >> 6. |
...
> Steps 2 and 3 must be run in order.
...
The following scripts can be run independently of Steps 1–3:
...
|--------|---------|-------|--------|
| `generate_figures.py` | Generate synthetic test figures | — | `lineformer_test/` |
